# if no temperature data it will take data from
...
def main():
    ...
    no_tmp_dates = find_no_tmp_dates(cursor)
    # pprint(no_tmp_dates)
    for el in no_tmp_dates:
        fill_tmp_gaps(el, st_distances, cursor)
    conn.commit()
    conn.close()


def fill_tmp_gaps(st_data, st_distances, cursor):
...
def find_no_tmp_dates(cursor):
    ...
    for st in tables:
        no_tmp_data = cursor.execute("""
            ...
            WHERE tmin is NULL or tmax is NULL or t is NULL or t='' or tmin='' or tmax=''
            ORDER BY dt
            """.format(st)).fetchall()
        if no_tmp_data:
            # print(st)
